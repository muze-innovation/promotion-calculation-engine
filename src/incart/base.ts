...
import ConditionTypes, { CategoryCondition, JsonConditionType, TagCondition } from './conditionTypes'
...
export abstract class InCartRule extends ARule {
  ...
  public constructor(uid: UID, priority: number, name: string, protected readonly conditions: JsonConditionType[]) {
    super(uid, priority, name)

    this.parsedConditions = this.conditions.map(condition => ConditionTypes.parse(condition, uid))
  }
  ...
  public getApplicableCartItems(buffer: CalculationBuffer): CartItem[] | 'all' {
    ...
    return buffer.filterApplicableCartItems(
      uids,
      {
