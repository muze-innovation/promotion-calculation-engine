...
import { Action, Condition, CalculatedCartItems, ItemDiscount, UID  } from "../index"
...
export default class BuyXGetYRule extends ARule {
  ...
  private getFreeItems(cartItems: CalculatedCartItems, freeQty: number): ItemDiscount[] {
  ...
  actions = [{
    perform: async (input: CalculationBuffer) => {
      ...
      const freeQty = (Math.floor(cartItems.totalQty / (this.x + this.y)) * this.y)
